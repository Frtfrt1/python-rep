...
result2[0].append(10)  # [0].append하면 둘 다 적용, .append하면 result2만 적용
...
result2 = copy.deepcopy(ori)  # 사본만 바뀜
...
nums2 = [5, 6]
print(nums + nums2) # 리스트 합치기 1

print([*nums, *nums2])  # 리스트 합치기 2
...
# [x] : x자체를 원소 하나로 해서 넣음
print([zip(subjects, scores)])  # zip object at <주소>, zip 앞에 * 붙이면 아래처럼 나옴

# list(x) : x를 순회해서 리스트에 넣음
print(list(zip(subjects, scores)))  # [('국어', 80), ('수학', 90), ('영어', 95)]
...
# [4, 16, 36, 64, 100]
result = [i**2 for i in range(1,11) if not i&1] # 조건은 뒤에
print(result)

# 1 ~ 10 중 짝수면 "짝", 홀수면 "홀" 출력하기
result = ["홀" if i&1 else "짝" for i in range(1,11)]   # 삼항은 앞에
print(result)
...
# x = 0 1 2
# y = 0 1 2
# x * y로 만들어진 리스트 [0, 0, 0, 0, 1, 2, 0, 2, 4]
...
result = sum(x for x in range(1, 101) if x%3 < 1 or x%5 < 1)
...
result = [x for x in words if x.find("n") >= 0]
...
result = [round(sum(x)/len(x), 2) for x in zip(*scores)]
