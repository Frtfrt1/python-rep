...
# 딕셔너리 언패킹
print(*d)   # key만 언패킹 됨
print(*d.values())   # value만 언패킹 됨
print(*d.items())   # tuple 형태로 언패킹 됨
...
d2 = {"eng": 90, "sci": 95}
print({**d,**d2})   # 뒤에 있는 거 우선 (update와 동일하게 동작)
...
result = dict((x,x**2) for x in range(1,11))
print(result)
...
result = dict(x for x in scores.items() if x[1] >= 90)
print(result)
...
result = {x[0]: "합격" for x in scores.items() if x[1] >= 60}
print(result)                                    # ✅ {'국어': '합격', '수학': '합격', '사회': '합격'}
...
print(result)                                    # ✅ {'국어': 90, '영어': 80, '수학': 100}
