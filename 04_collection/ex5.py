...
# 딕셔너리 순회
for key in d:
    print(key, d[key]) # key만 나옴

for i, data in enumerate(d):
    print(i, data)  # 여기서 i는 인덱스가 아니고 순회하는 순서
...
a = d.items()
d["key"] = 100
print(a)    # a는 d의 view여서 변경이 반영됨

# ===========================================================
# 3. 딕셔너리는 sequence 객체가 아니다. (인덱싱, 슬라이싱 불가)
# ===========================================================

d[0] = "python" # 0은 인덱스가 아니라 새로운 key값임
print(d)
...
print(hash("hash")) # string 포함되면 실행할 때마다 hash값 달라짐
print(hash(12342))
print(hash((2,4,)))
...
print(len(d))
print(sum(d.values()))
print(min(d), min(d.values()))   # eng 80 (그냥 d하면 key값만 해당)
print(max(d), max(d.values()))   # mat 90 (그냥 d하면 key값만 해당)

print(sorted(d))    # key만 sort
print(dict(sorted(d.items())))    # key 기준으로 (key, value)를 오름차순 sort
print(dict(sorted(d.items(), reverse=True)))    # key 기준으로 (key, value)를 내림차순 sort
