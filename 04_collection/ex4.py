...
for i in gen:   # gen은 iterator랑 비슷해서 위에 1,2,3 나오고 여기서부터 4,5,6,... 나옴
    print(i, end=" ")
print()
...
print(sum(a), sum(a))   # 55 55
print(sum(b), sum(b))   # 55 0
...
p1 = (1, 2, 3)
p2 = (10, 20, 30)
result = tuple(sum(x) for x in zip(p1, p2))
print(result)
...
print(f"{sum(hours[x] for x in range(1,6))}시간")                                                    # ✅ 15시간
# sum(hours[1:6])
...
# print(f"max 점수: {max(result[-1])}점, min 점수: {min(result[0])}점")                                                    # ✅ max 점수: 92점, min 점수: 76점
...
print(f"총액: {sum(x[1]*x[2] for x in stocks):,}원")                                                    # ✅ 총액: 21,000원
...
total = sum(x[1]*x[2] for x in zip(*stocks))
print(f"총액: {total:,}원")